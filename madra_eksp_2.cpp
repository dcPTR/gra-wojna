...
#define FALSZ 0
#define NIKT 404
#define INSTRUKCJA_SPRAWDZ_KONIEC switch(sprawdzKoniec(gracz)){case 1:return GRACZ2; case 2:return GRACZ1;}
...
int wojnaA(Gracz gracz[], Karta kartaGracz1, Karta kartaGracz2, Karta kartyWojna[], int* iloscKartWojna, int* iloscRuchow, int dwieKartyMaGracz){
    ...
    *iloscKartWojna += 2;
    ...
    dwieKartyMaGracz = NIKT; // od tej chwili toczy się zwykła wojna
    ...
    *iloscKartWojna += 2;
    ...
    if(kartaGracz1.figura > kartaGracz2.figura){
        ...
        for(int i=0; i<*iloscKartWojna; i+=1){
...
int strategia(Karta mojeKarty[2], Karta kartaPrzeciwnika, char wybor){
    switch(wybor){
        case 'L':
            return rand()%2;
            break;
        case 'W':
            for(int i=0 ;i<2; i++){
                if(mojeKarty[i].figura == kartaPrzeciwnika.figura){
                    return i;
                }
            }
            if(mojeKarty[0].figura > kartaPrzeciwnika.figura){
                if(mojeKarty[1].figura > kartaPrzeciwnika.figura){
                    if(mojeKarty[0].figura < mojeKarty[1].figura){
                        return 0;
                    }
                    return 1;
                }
            ...
            else if(mojeKarty[1].figura > kartaPrzeciwnika.figura){
                return 1;
            }
            else{
                if(mojeKarty[0].figura < mojeKarty[1].figura){
        ...
        case 'P':
            ...
            else if(mojeKarty[1].figura < mojeKarty[0].figura){
                if(mojeKarty[1].figura != kartaPrzeciwnika.figura){
                    return 1;
                }
                return 0;
...
int gra(Gracz gracz[], int* iloscRuchow, int wielkoscTalii){
    ...
    int iloscKartWojna = 0, wynikWojny = 0, dwieKarty = GRACZ2, numerKartyWojna, bylaWojna = 1;
        ...
        if(bylaWojna == FALSZ){
            if(dwieKarty == GRACZ1){ // teraz pobiera dwie karty
                if(gracz[0].reka.ilosc_kart >= 2){
                    ...
                    numerKartyWojna = strategia(karty1, kartaGracz2, 'W');
                ...
                if(gracz[1].reka.ilosc_kart >= 2) {
                    ...
                    kartaGracz1 = karty1[!numerKartyWojna];
                    numerKartyWojna = strategia(karty2, kartaGracz1, 'P');
                    kartaGracz2 = karty2[numerKartyWojna];
                }
                else{
                    kartaGracz1 = pobierzKarte(&gracz[0]);
...
int main() {
    ...
    int iloscRuchow = 0, wygrana = 0, wielkoscTalii = 52, avg = 0, wygraneGracz1 = 0, wygraneGracz2 = 0;
    ...
    srand(time(NULL));
        ...
        for(wielkoscTalii = 52; wielkoscTalii <= 52; wielkoscTalii +=4){
            wygraneGracz1 = 0;
            wygraneGracz2 = 0;
            ...
            cout<<"Wygrane: "<<wygraneGracz1<<" vs "<<wygraneGracz2<<endl;
        }
    return 0;
}