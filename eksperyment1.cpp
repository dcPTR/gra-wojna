...
int wojnaA(Gracz gracz[], Karta kartaGracz1, Karta kartaGracz2, Karta kartyWojna[], int* iloscKartWojna, int* iloscRuchow){
    switch(sprawdzKoniec(gracz)){
        case 1:
            return 2; //wygrał gracz 2
        case 2:
            return 1; //wygrał gracz 1
    }
...
int main() {
    ...
    for(int i=26; i<52; i++){
        ...
        talia.karty[i].figura = t2[(i-26)*2+1];
            ...
            if(wygrana == GRACZ1){
                if(DEBUG) cout<<endl<<"Koniec gry. Wygrał gracz 1."<<endl;
            }
            else if(wygrana == GRACZ2){
                if(DEBUG)cout<<endl<<"Koniec gry. Wygrał gracz 2."<<endl;
            }
